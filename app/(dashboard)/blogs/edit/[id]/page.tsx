...
import { Link as LinkIcon, ChevronLeft, Loader2, Save, X, ArrowRight } from "lucide-react";
...
import { fetchBlog, updateBlog, getAllNextSteps } from "@/app/actions/blogOps";
...
const cat = Object.values(BlogCategory)
...
export default function EditBlogPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [categories, setCategories] = useState<BlogCategory[]>(Object.values(BlogCategory));
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const [blog, steps] = await Promise.all([
          fetchBlog(id),
          getAllNextSteps()
        ]);
        ...
        if (blog) {
          form.reset({
            ...
            audience: blog.audience as BlogAudience,
            image: blog.image || "",
            ...
            categories: (blog as any).categories || [],
            nextSteps: (blog as any).nextSteps?.map((ns: any) => ({ id: ns.id })) || [],
            showCallout: blog.showCallout || false,
          });
        }
  ...
  return (
    <div className="container mx-auto py-8 px-6 min-h-full tap-dark">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900 font-poppins">Edit Blog Post</h1>
      </div>
                  ...
                  <div className="relative">
                    ...
                    {isCategoryOpen && (
                      <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                        {categories
                          .filter(c => c.toLowerCase().replace(/_/g, " ").includes(categorySearch.toLowerCase()))
                          .map((catValue) => (
                            <div
                              key={catValue}
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 p-4 bg-gray-50 rounded-xl border border-gray-100">
                    {Object.values(BlogCategory).map((catValue) => {
                      const isPrimary = form.watch("category") === catValue;
                      return (
                        <div key={catValue} className="flex items-center space-x-2">
                          <Checkbox
                            id={`cat-${catValue}`}
                            checked={field.value?.includes(catValue) || isPrimary}
                            disabled={isPrimary}
                            onCheckedChange={(checked: boolean) => {
                              const current = field.value || [];
                              if (checked) {
                                field.onChange([...current, catValue]);
                              } else {
                                field.onChange(current.filter((c) => c !== catValue));
                              }
                            }}
                          />
                          <label
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex items-center space-x-2">
                  ...
                  <FormLabel className="text-sm text-gray-700 cursor-pointer pt-2">Published</FormLabel>
                </FormItem>
              )}
            />
              ...
              {availableNextSteps.length > 0 ? (
                availableNextSteps.map((step) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-4 h-4 rounded border flex items-center justify-center ${isSelected ? "bg-[#5C9952] border-[#5C9952]" : "border-gray-300"
                        }`}>
